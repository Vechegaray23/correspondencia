...
export async function createPaquete(req, res) {
    ...
    try {
       ...
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, destinatario`, [depto, receptor, destinatario, comentarios, urgencia]);
        const pkg = pkgRows[0];
        ...
        if (userRows.length) {
            ...
            nuevoPaquete({ id: pkg.id, destinatario: pkg.destinatario, phone }, email).catch(err => console.error('Error notificación nuevoPaquete:', err));
        }
    ...
    try {
      ...
      RETURNING id, destinatario,estado, depto`, [estado, id]);
        ...
        if (userRows.length) {
            ...
            estadoActualizado({ id: pkg.id, estado: pkg.estado, phone }, email).catch(err => console.error('Error notificación estadoActualizado:', err));
        }
        return res.json(pkg);
    }
    catch (err) {
